...
# Functions
class StartNmap(threading.Thread):
    ...
    def T1_reply(self, payload, num):
        ...
        send(newpkt, iface=self.iface, verbose=0)

    # T5 response
    def T5_reply(self, payload):
        global IPID
        pkt = ip.IP(payload.get_payload())
        IPID = IPID + 1
    ...
    def callback(self, pl):
        ...
        if pkt.p == ip.IP_PROTO_TCP:
            options = binascii.hexlify(pkt.tcp.opts)
            flags = self.tcp_flags(pkt.tcp.flags)
    ...
    def run(self):
        ...
        os.system('iptables -A INPUT -i ' + self.iface + ' -j NFQUEUE --queue-num 4')
        q = nfqueue.NetfilterQueue()
        q.bind(4, self.callback)
